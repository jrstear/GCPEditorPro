# GCPEditorPro Fork Changes — `feature/auto-gcp-pipeline`

This branch contains a set of UX and data-model changes to improve GCPEditorPro in general, and specifically for an automated drone-photogrammetry pipeline in a separate repository:
[**jrstear/geo**](https://github.com/jrstear/geo) — see
[`docs/odm-workflow.md`](https://github.com/jrstear/geo/blob/main/docs/odm-workflow.md)
...
| `src/app/gcps-map/gcps-map.component.ts` | Progress badges; summary line; labeled progress-pin in pipeline mode (color reflects tagged-image count, label text shows the GCP/CHK name) |
...
- Rename "Ground Control Points" on the target list page to "Targets", since
  the list now contains both GCP and CHK points.
- Consider renaming GCPEditorPro itself to something like TargetTaggerPro to
  reflect that the tool is no longer GCP-only.

## Running this fork from source

Requires the OpenSSL legacy provider (Node 17+):
...
This fork inherits the upstream Fair Source License from
[uav4geo/GCPEditorPro](https://github.com/uav4geo/GCPEditorPro). See
